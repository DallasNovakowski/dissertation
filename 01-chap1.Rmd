...
The experiments across both Chapters 1 and 2 will be conducted in the context of a two-player economic game, called the security game [@tambe2011]. At its base form, the game is one-shot, extensive-form, and conducted with complete information. In this modification of the security game, participants are paired with an anonymous partner, with each being given an endowment of funds (e.g., \$2 each; \$300 for large stakes). Then, participants are told that their partner will have the opportunity to try to steal money from them (e.g., \$1 at a 50% chance of success, with failed thievery attempts instead making the partner lose \$1; \$150 for large stakes). Together, the incomes, purchasing of security, and decision to attempt a risky theft come together to determine the players' final payoffs ($\pi_i$)

After learning this information, participants can spend some of their money on security products (e.g., \$0.02 per unit), with each unit of security visibly reducing the probability of a successful thievery attempt (1% reduction per unit).
...
As seen in Figure \@ref(fig:s1a-security-jitt) and Table \@ref(tab:s1a-security-anova)) the data did not indicate any significant effect of inequality on security consumption, `r anova_extract(ineq_anova_mixed,"inequality")`, failing to reject **H1~0~)**. However, stake size had a significant and positive effect on security consumption, `r anova_extract(ineq_anova_mixed,"stake_cond")`. The effect of stake size was not predicted, but may reflect increased risk aversion for larger values - participants seemed willing to risk lose trivial quantities.

not which is interesting since the price of security was proportional to the stake size. 
...
The manipulation check suggests that inequality does indeed increase perceived envy, as predicted `r anova_extract(ineq_anova_mixed_envy,"inequality")`. In addition, a significant interaction was found between stake size and inequality; inequality led to more perceived envy when stake sizes were large `r anova_extract(ineq_anova_mixed_envy,"inequality:stake_cond")`. The higher ratings of envy in the inequality condition suggest that inequality was noticeable by participants, and that they had some expectation that inequality impacted their partner. See Figure \@ref(fig:s1a-envy-jitt) and Table \@ref(tab:s1a-envy-anova).
...
```
```{r s1a-envy-jitt, eval = TRUE, include=TRUE, echo=FALSE, out.width='70%'}
...
Mirroring the effects on security consumption, a 2b\*2w ANOVA failed to find an effect of inequality on perceived likelihood of the partner attacking, `r anova_extract(ineq_anova_mixed_likelihood,"inequality")`. However, stake size still had a significant and positive effect on perceived attack likelihood, `r anova_extract(ineq_anova_mixed_likelihood,"stake_cond")`. See Figure \@ref(fig:s1a-likelihood-jitt) and Table \@ref(tab:s1a-likelihood-anova). Although inequality led to increased perceived envy, this anticipation of envy does cascade into expecting malicious intentions from one's partner.
...
The results of Study 1a are discouraging for the proposed theory of envy anticipation. In this experiment, inequality did cause participants to anticipate envy in their partner, but they do not fear any behavioural reaction from this partner. The null results in Study 1a might be attributable to the complete-information nature of the security game. The presence of explicit probabilities and payoffs game may have distracted participants from considering the larger social context of their decision. 

For instance, some of the comments from participants explicitly stated that they were attempting to maximize expected values. However, such efforts to calculate seem to emphasize the numbers presented, with little consideration whether the surrounding context made their partner more or less likely to attack. As such, Study 1b made a number of methodological changes to decrease any potentially distracting or artificial features of the task, and to increase the potential salience of inequality.
...
Study 1b sought to strictly replicate its pilot study, with no changes except pre-registration (LINK ??) and a larger sample. Based on a pre-registered power analysis targeting *d* = 0.2 with 95% power for a paired t-test design, `r s1b_n_collected` participants were recruited, in anticipation of a 10% exclusion rate. After exclusions, n = `r s1b_n_attended` participants were retained for analyses (`r round(s1b_female*100,2)`% female). 
...
Supporting **H1)**, in the inequality condition, Study 1b found that participants spent an average of \$`r round(s1b_spend_t_tidy$mean_difference,2)` more on security products compared to the no inequality condition, `r report_tidy_t(s1b_spend_t_tidy)`, corresponding to a medium effect size (Figure \@ref(fig:s1b-spending-jitt)). Likewise,  a mediation analysis found a significant and positive indirect effect of inequality on security spending through perceived partner envy, `r med_extract(s1b_spending_med_full_tidy,"Indirect Effect (ACME)")`, supporting **H2)** (see Figure \@ref(fig:s1b-spending-med-plot) and Table \@ref(tab:s1b-spending-med-full)). Consistent with trends observed in the pilot data, the mediation analysis indicated a partial mediation by perceived partner envy; the direct effect of inequality on security spending remained positive, `r med_extract(s1b_spending_med_full_tidy,"Direct Effect (ADE)")`. 
...
```
```{r s1b-spending-jitt, eval = TRUE, include=TRUE, echo=FALSE, out.width='70%'}
...
One possible explanation for the significant direct effect is that the inequality manipulation is confounded with the Attacker's absolute level of income, and the diminishing marginal utility of money at higher values. Since Attackers have less money overall in the high-inequality condition, they might prefer greater risk compared to when they have more money, since a dollar increase from \$150 to  \$151 may be preferred compared to a dollar increase from \$300 to \$301. A future study could manipulate whether the Defender's income is visible to the Attacker in a 2(inequality)*2(income visibility) design. A positive and significant interaction between inequality and visibility would suggest that any (expectation of) risk-taking is not due to just the Attacker having less money, but rather because the Attacker was able to make a social comparison.

Another mechanism could be that participants purposely paid for security in order to reduce their money, and thus reduce inequality. However, this latter narrative is difficult disentangle because some participants explicitly stated that they purchased *less* security so that their partner could take their money, and reduce inequality. Such a possibility raises an intriguing question for future studies of whether a pre-security option of transferring wealth to one's partner might reduce overall security spending.


### Study 1c 

Following a power analysis for an independent-samples t-test, n = 1204 participants were collected, in anticipation of a 10\% exclusion rate, targeting n = 1084 for an analysis to detect an effect of d = .2 with .95 power.
